...
func UpdateDebugText():
	...
	AddText(debugText,"IsHooked: " + str(hook.isHooked));
	AddText(debugText,"Hook Velocity: " + str(hook.velocity));
	AddText(debugText,"Hook Collided: " + str(hook.collider.disabled));
	...
	AddText(debugText,"Chain length: " + str(chain.spring.length));
	...
	AddText(debugText, "Chain.spring node_b: " + PosAsStr(playerNode.global_position));
	AddText(debugText);
